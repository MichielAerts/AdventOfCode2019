...
fun main() {
    ...
    // first part
//    val output = program.runProgram(1L)

    // second part
    val output = program.runProgram(2L)
...
class IntCode(
    ...
    fun runProgram(firstInput: Long): Long? {
//        println(program.subList(0, 100))
        val input = firstInput
        var output: Long? = null
        var count = 0
        while (Operation.createOp(program[pos]).opCode != 99) {
            val op = Operation.createOp(program[pos])
            output = performOperation(op, input)
            if (count++ % 1000 == 0) println(count)
//            println("relative base: $relativeBase, output: $output, new pos: $pos, program: ${program.subList(0, 100)}")
        }
        return output
    }
